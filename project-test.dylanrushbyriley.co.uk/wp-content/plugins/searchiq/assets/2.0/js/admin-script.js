...
$jQu(document).ready(function(){
			...
			$jQu.ajax({
				...
				if(res.success == 0){
					...
					setTimeout(function () {
						...
						//thisVar.parents('.section').find('h2').trigger('click');
						if (typeof res.engineName != 'undefined') {
							/*thisVar.parents('.section').next('.section.section-2').find("#siq_search_engine").val(res.engineName);
							 thisVar.parents('.section').next('.section.section-2').removeClass('not-done').addClass('done no-engine');
							 $jQu('#btnSubmitEngine').trigger('click');*/
					...
					//thisVar.parents('.section').next('.section.section-3').removeClass('not-done').addClass('done').addClass('not-indexed');
				}
			});
		}else{
	...
	$jQu('#btnSubmitAuth').click(function(){
		var thisVar = $jQu(this);

		var errorMsg = "";
		thisVar.addClass('ajaxLoading');
		var username = $jQu("#siq_username").val();
		var password = $jQu("#siq_password").val();
		if(username != "" && password != ""){
			thisVar.parents('.data').find('div.msg').remove();
			$jQu.ajax({
				method: "POST",
				url: adminAjax,
				data: { action: "siq_ajax", task: "verify_auth", username: username, password: password, security:window.siq_admin_nonce },
				dataType: 'json',
				crossDomain: true
			}).done(function( response ) {
				thisVar.removeClass('ajaxLoading');
				...
				if(typeof res.success != "undefined") {
					...
					thisVar.parents('.data').append('<div class="msg errorMsg">There was some issue with the request. Please try again.</div>');
				}
			});
		}else{
			thisVar.parents('.data').find('.errorMsg').remove();
			...
			thisVar.parents('.data').append('<div class="msg errorMsg">Please enter '+errorMsg+' </div>');
			thisVar.removeClass('ajaxLoading');
		}
	});
	...
	$jQu('#btnSubmitEngine').click(function(){
        ...
        $jQu.ajax({
            ...
            if(res.success == 0){
                ...
                //thisVar.parents('.section').removeClass('not-done').addClass('done');
                //thisVar.parents('.section').find('h2').trigger('click');
            }
        });
	});
...
function syncPosts(a){
	...
	$jQu.ajax({
			...
			if(typeof res.searchengine != "undefined" && res.searchengine == false){
				setTimeout(function(){
					if(thisVar.parents(".section").hasClass("section-3")){
						...
						thisVar.parents(".section").nextAll('.section').addClass('not-done');
					}else{
						...
						prevParent.nextAll('.section').addClass('not-done');
		...
		thisVar.parents('.data').append('<div class="msg errorMsg">There was some error processing your request, please try later.</div>');
		thisVar.removeProp("disabled");
		setProp(thisVarParent, false);
		if($jQu(".messageIndexPosts").size() > 0){
			$jQu(".messageIndexPosts").remove();
		...
		step: function( now, fx ){
			...
			if (isSync) {
				if(msg != ''){
					...
					window.setTimeout(function(){
						//el.parents('.section').find('.data').css('display','none');
...
function syncPostsOnFacets(a){
	...
	$jQu.ajax({
		...
		currentSection.find('.data').append('<div class="msg errorMsg">There was some error processing your request, please try later.</div>');
		setProp(thisVarParent, false);
		currentSection.find('.progressText').hide();
		currentSection.find('.progress-wrap.progress').hide();
	});
