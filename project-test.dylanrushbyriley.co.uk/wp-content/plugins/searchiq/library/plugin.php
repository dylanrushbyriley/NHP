...
class siq_plugin extends siq_shortcode{
	...
	public function __construct(){
		...
		$this->administerPanelLink		= $this->clearLink(SIQ_SERVER_BASE.SIQ_SERVER_SUB_FOLDER."puser/create-domain.html", true);							// link to the user's dashboard on searchiq.xyz
		...
		if($this->getSearchQueryParamName() != ""){
			$this->siq_custom_get_param = $this->getSearchQueryParamName();
		}else{
	...
	public function getSIQPluginSettings(){
		...
		foreach($this->pluginSettings as $k => $v){
			...
			{
				if(strcmp($k,'custom_search_page') == 0 || strcmp($k,'custom_search_page_old') == 0 &&  !empty($v))
				{
					$v = get_permalink($v);
				}
...
<a href='" . get_admin_url() . "admin.php?page=dwsearch&showIconNotice=0' class='disableAdminNotice siqNoticeDismiss'>Dismiss</a></div>";
	...
	public function add_body_class($classes){
		global $post;
		if($this->is_custom_search_page_set() && !empty($post) && $this->pluginSettings['custom_search_page'] == $post->ID){
	...
	public function check_if_search(){
		global $post;
		$searchQuery = rawurlencode(stripslashes( get_search_query(  ))) OR $searchQuery =@ rawurlencode(stripslashes(sanitize_text_field($_GET[$this->getSearchboxName()])));
		$searchQuery .= "";
		if((is_search() || isset($_GET[$this->getSearchboxName()]) && (is_home() || (!empty($post) && $this->pluginSettings['custom_search_page'] != $post->ID))) && $this->is_custom_search_page_set() && !isset($_GET['siq_e']) && $searchQuery != "" && $this->pluginSettings['engine_code'] != ""){
			if($this->checkIfServerWorking()){
				$resultOnOff = isset($_GET['result']) ? "&result=" . urlencode($_GET['result']) : "";
				$redirect = get_permalink($this->pluginSettings['custom_search_page']);
			...
			set_query_var('s', $_GET[$this->siq_custom_get_param]);
		}
	...
	public function siq_ajax_search(){
		global $post;
		$content = "";
		if(($this->is_custom_search_page_set() && $this->pluginSettings['custom_search_page'] == $post->ID) || (isset($_GET['result']) && $_GET['result'] == 'on')){
...
EOF;
	...
	public function createAdminMenu(){
		add_menu_page(__('searchIQ', 'dwsearch'),__('searchIQ', 'dwsearch'),'manage_options', 'dwsearch', array($this, "config"));
	}
	...
	public function createCustomSearchPageIfNotExists() {
		...
		}else if($customSearchPage != ""){
			$pageData = get_post($customSearchPage);
			if(!is_null($pageData) && has_shortcode($pageData->post_content, 'siq_ajax_search') && $pageData->post_status == 'publish'){
				$customSearchPage 	= $customSearchPage;
				update_option($this->pluginOptions['custom_search_page'], $customSearchPage);

			}else if($customSearchPageOld != ""){
				$pageData = get_post($customSearchPageOld);
				if(!is_null($pageData) && has_shortcode($pageData->post_content, 'siq_ajax_search') && $pageData->post_status == 'publish'){
	...
	public function setCustomSearchPage($data) {
		$updated = true;
		if (!empty($data["id"]) && $this->is_valid_custom_search_page($data["id"])) {
			update_option($this->pluginOptions["custom_search_page"], $data["id"]);
			$this->pluginSettings["custom_search_page"] = $data["id"];
		} else {
			$updated = false;
		}
		...
		if (empty($this->pluginSettings["custom_search_page"])) {
			$selectPageList 	.= '<option value="" selected="selected">--Select Page--</option>';
		}
		foreach($allpages as $k => $v){
			$selected		= ($this->pluginSettings["custom_search_page"] !="" && $this->pluginSettings["custom_search_page"] == $v->ID )? 'selected="selected"': "";
	...
	public function verify_auth($data){
		$result     = array();
		$code 	    = "";
		$message    = "There is some issue with the";
		if(trim($data["username"]) != "" && trim($data["password"]) != ""){
			$result = parent::verify_auth($data);
			if(strtolower($result['message']) === "ok"){
				$res =  $result['response'];
				if($res['apiKey'] != ""){
					$resData['api_key'] = $res['apiKey'];
					$result = $this->verify_subscription($resData);
				}
	...
	public function delta_sync_posts($data, $total = 0){
		...
		if($this->totalPosts == 0 && $data['total'] == 0){
			//Please check deleted records 
				...
				$this->log_error("UPDATED POSTS",$logError);		
				$this->log_error($this->deltaPostIDs,$logError);		
				
				$deletePostIds			= parent::getSyncPostIds('d'); //delta_sync_posts  GET ALL DELETED POSTS for SYNC here	
				$this->log_error("DELETED POSTS",$logError);		
				$this->log_error($deletePostIds,$logError);		
				...
				$this->log_error("ONLY FOR UPDATION",$logError);	 
				$this->log_error(print_r($postIDs, true),$logError);	// Log only those posts which are to be updated
				...
				if(is_array($deletePostIds) && count($deletePostIds) > 0){ //IF posts available for deletion bulk delete from searchIQ
					...
					$this->log_error("BULK-DELETED",$logError);
					$this->log_error(print_r($deleteBulk, true),$logError);	
		...
		$this->log_error("DELTA-POSTS",$logError);	 
		$this->log_error($this->deltaPostIDs,$logError);		// Log delta post ID's
		if($this->totalPosts > 0){
			if(isset($data['currentPage']) && $data['currentPage'] > 0){
			...
			$excludePosts = (!empty($this->excludePostIds)) ? " AND p.ID NOT IN(".$this->excludePostIds.") " : "";
			$query = "SELECT p.*, GROUP_CONCAT(pm.meta_key SEPARATOR '<>') as meta_key, GROUP_CONCAT(pm.meta_value SEPARATOR '<>') as meta_value FROM ".$wpdb->prefix."posts p LEFT JOIN ".$wpdb->prefix."postmeta pm ON p.ID = pm.post_id WHERE p.ID IN(".$this->deltaPostIDs.") ".$excludePosts." GROUP BY p.ID ORDER BY p.ID ASC limit 0,".$this->postsPerCallForDeltaSync." ";		  
			$dataResult = $wpdb->get_results($query);
			
			$this->log_error($query, $logError);		// Log QUERY based on current page
			...
			if(!empty($dataResult)){
				...
				$this->log_error($result,  $logErro);		
				if($result['success'] == true){
					parent::addToSync($idsForSubmission);  // Add posts to local DB for tracking delta
					$this->log_error("IDS-TO-SYNC", $logError);
					$this->log_error($idsForSubmission, $logError);
	...
	public function full_sync_posts($post){
		...
		$this->log_error("==== FULL SYNC POSTS====", $log);
		$this->log_error($result, $log);
		return $result;
	}
	...
	public function submit_for_indexing($data, $total = 0){
		...
		if($this->totalPosts > 0){
			...
			$excludePosts = (!empty($this->excludePostIds)) ? " AND p.ID NOT IN(".$this->excludePostIds.") " : "";
			if($this->syncPostIDs != ""){
				$query = "SELECT p.*, GROUP_CONCAT(pm.meta_key SEPARATOR '<>') as meta_key, GROUP_CONCAT(pm.meta_value SEPARATOR '<>') as meta_value FROM ".$wpdb->prefix."posts p LEFT JOIN ".$wpdb->prefix."postmeta pm ON p.ID = pm.post_id WHERE p.ID IN(".$this->syncPostIDs.") ".$excludePosts." GROUP BY p.ID ORDER BY p.ID ASC limit ".$this->currentStart.",".$this->postsPerCall." ";
			}else{
				$query = "SELECT p.*, GROUP_CONCAT(pm.meta_key SEPARATOR '<>') as meta_key, GROUP_CONCAT(pm.meta_value SEPARATOR '<>') as meta_value FROM ".$wpdb->prefix."posts p LEFT JOIN ".$wpdb->prefix."postmeta pm ON p.ID = pm.post_id WHERE TRIM(p.post_title) <> '' AND p.post_password = '' ".$excludePosts." ".$allowedPostTypesFilter." ".$postStatus." GROUP BY p.ID ORDER BY p.ID ASC limit ".$this->currentStart.",".$this->postsPerCall." ";
			}
			...
			if($result['success'] == true && array_key_exists("errors", $result) && count($result['errors']) == 0){
				...
				if($this->currentPage < $this->totalPages){
					...
					$result['facets']   = $this->removeFacetsNotice();
